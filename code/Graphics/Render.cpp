...
void Render::drawTexture(float x, float y, TextureEx1* texture, glm::vec2* texcoords, ::Color& color, float curframe, int animW, int animH)
{
...
//    glDrawBuffers(1, x);
    ...
    std::vector<char>SkeletalModelWithShadowVertFile;
    std::vector<char>SkeletalModelWithShadowFragFile;
    SkeletalModelWithShadowVertFile.insert(SkeletalModelWithShadowVertFile.end(), SkeletalModelWithShadow_vert_glsl, SkeletalModelWithShadow_vert_glsl + strlen(SkeletalModelWithShadow_vert_glsl));
    SkeletalModelWithShadowFragFile.insert(SkeletalModelWithShadowFragFile.end(), SkeletalModelWithShadow_frag_glsl, SkeletalModelWithShadow_frag_glsl + strlen(SkeletalModelWithShadow_frag_glsl));

//    auto&& SkeletalModelWithShadowVertFile = res.Res("Shader/SkeletalModelWithShadow.vert.glsl");
//    auto&& SkeletalModelWithShadowFragFile = res.Res("Shader/SkeletalModelWithShadow.frag.glsl");
    if (shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_ANIM_MODEL_WITH_SHADOW] == nullptr) {
        shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_ANIM_MODEL_WITH_SHADOW] = new Shader(
                SkeletalModelWithShadowVertFile, SkeletalModelWithShadowFragFile);
    }

    std::vector<char>ShadowMappingDepthSkeletalVertFile;
    std::vector<char>ShadowMappingDepthSkeletalFragFile;
    ShadowMappingDepthSkeletalVertFile.insert(ShadowMappingDepthSkeletalVertFile.end(), ShadowMappingDepthSkeletal_vert_glsl, ShadowMappingDepthSkeletal_vert_glsl + strlen(ShadowMappingDepthSkeletal_vert_glsl));
    ShadowMappingDepthSkeletalFragFile.insert(ShadowMappingDepthSkeletalFragFile.end(), ShadowMappingDepthSkeletal_frag_glsl, ShadowMappingDepthSkeletal_frag_glsl + strlen(ShadowMappingDepthSkeletal_frag_glsl));

//    auto&& ShadowMappingDepthSkeletalVertFile = res.Res("Shader/ShadowMappingDepthSkeletal.vert.glsl");
//    auto&& ShadowMappingDepthSkeletalFragFile = res.Res("Shader/ShadowMappingDepthSkeletal.frag.glsl");
    if (shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_DEPTH_ANIM] == nullptr) {
        shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_DEPTH_ANIM] = new Shader(
                ShadowMappingDepthSkeletalVertFile, ShadowMappingDepthSkeletalFragFile);
    }
...
//    auto&& DebugShadowMappingFragFile = res.Res("Shader/DebugShadowMapping.frag.glsl");
    if (shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_DEBUG] == nullptr) {
        shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_DEBUG] = new Shader(DebugShadowMappingVertFile,
                                                                        DebugShadowMappingFragFile);
    }


    std::vector<char>SkeletalModelWithShadow;
    std::vector<char>LightFragFile;
    SkeletalModelWithShadow.insert(SkeletalModelWithShadow.end(), SkeletalModelWithShadow_vert_glsl, SkeletalModelWithShadow_vert_glsl + strlen(SkeletalModelWithShadow_vert_glsl));
    LightFragFile.insert(LightFragFile.end(), Light_frag_glsl, Light_frag_glsl + strlen(Light_frag_glsl));

//    auto&& SkeletalModelWithShadow = res.Res("Shader/SkeletalModelWithShadow.vert.glsl");
//    auto&& LightFragFile = res.Res("Shader/Light.frag.glsl");
    if (shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_LIGHT] == nullptr) {
        shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_LIGHT] = new Shader(SkeletalModelWithShadow,
                                                                        LightFragFile);
    }

    std::vector<char>blurVertFile;
    std::vector<char>blurFragFile;
    blurVertFile.insert(blurVertFile.end(), blur_vert_glsl, blur_vert_glsl + strlen(blur_vert_glsl));
    blurFragFile.insert(blurFragFile.end(), blur_frag_glsl, blur_frag_glsl + strlen(blur_frag_glsl));

//    auto&& blurVertFile = res.Res("Shader/blur.vert.glsl");
//    auto&& blurFragFile = res.Res("Shader/blur.frag.glsl");
    if (shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_BLUR] == nullptr) {
        shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_BLUR] = new Shader(blurVertFile,
                                                                       blurFragFile);
    }

    std::vector<char>bloomFinalVertFile;
    std::vector<char>bloomFinalFragFile;
    bloomFinalVertFile.insert(bloomFinalVertFile.end(), bloomFinal_vert_glsl, bloomFinal_vert_glsl + strlen(bloomFinal_vert_glsl));
    bloomFinalFragFile.insert(bloomFinalFragFile.end(), bloomFinal_frag_glsl, bloomFinal_frag_glsl + strlen(bloomFinal_frag_glsl));

//    auto&& bloomFinalVertFile = res.Res("Shader/bloomFinal.vert.glsl");
//    auto&& bloomFinalFragFile = res.Res("Shader/bloomFinal.frag.glsl");
    if (shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_BLOOM_FINAL] == nullptr) {
        shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_BLOOM_FINAL] = new Shader(bloomFinalVertFile,
                                                                              bloomFinalFragFile);
    }

    std::vector<char>S2DVertFile;
    std::vector<char>S2DFragFile;
    S2DVertFile.insert(S2DVertFile.end(), s2d_vert_glsl, s2d_vert_glsl + strlen(s2d_vert_glsl));
    S2DFragFile.insert(S2DFragFile.end(), s2d_frag_glsl, s2d_frag_glsl + strlen(s2d_frag_glsl));

//    auto&& S2DVertFile = res.Res("Shader/2d.vert.glsl");
//    auto&& S2DFragFile = res.Res("Shader/2d.frag.glsl");
    if (shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_2D] == nullptr) {
        shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_2D] = new Shader(S2DVertFile,
                                                                     S2DFragFile);
    }
...
//    auto&& shaderFragFile = res.Res("Shader/shader.frag.glsl");
    if (shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_2D2] == nullptr) {
        shader[SHADER_LOGIC_TYPE::SHADER_LOGIC_TYPE_2D2] = new Shader(shaderVertFile,
                                                                      shaderFragFile);
    }
    ...
    glBindRenderbuffer(GL_RENDERBUFFER, wrbo);//TODO err?
    glDrawBuffers(2, attachments);
//
    int ret = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    if (ret != GL_FRAMEBUFFER_COMPLETE) {
        std::cout << "Framebuffer not complete! err:" << ret << std::endl;
    }
    glBindFramebuffer(GL_FRAMEBUFFER, wfbo);
...
//
    if(1)
    {
        ...
        //FIXME//glTexParameterfv(GL_TEXTURE_2D, GL_TEXTURE_BORDER_COLOR, borderColor);
