...
//    auto particlevs = res.Res("Shader/Particle.vert.glsl");
//    auto particlefs = res.Res("Shader/Particle.frag.glsl");
//    particleShader = new Shader(particlevs, particlefs);
    std::vector<char>particlevs;
    std::vector<char>particlefs;
    particlevs.insert(particlevs.end(), Particle_vert_glsl, Particle_vert_glsl + strlen(Particle_vert_glsl));
    particlefs.insert(particlefs.end(), Particle_frag_glsl, Particle_frag_glsl + strlen(Particle_frag_glsl));
    particleShader = new Shader(particlevs, particlefs);
    particleShader->use();
    ...
    tex = new TextureEx1(GL_TEXTURE_2D, "p.png", IMAGE);
